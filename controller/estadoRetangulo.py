...
class EstadoRetangulo(Ferramenta):
    ...
    def iniciar_figura_nova(self, event, executor):
        ...
        #O Estado cria o retângulo, mas guarda no Executor.
        executor.figuraAtual = Retangulo(coordenadas, corBorda, corPreenchimento, dash=(4,2))
    ...
    def incluir_figura_nova(self, event, executor):
        if executor.figuraAtual is not None:
            #Finalizando o retângulo (tira o tracejado - dash):
            executor.figuraAtual.finalizar()
            
            #Guardando o retângulo na lista de desenhos do Executor:
