...
class EstadoElipse(Ferramenta):
    ...
    def iniciar_figura_nova(self, event, executor):
        ...
        #O Estado cria a elipse, mas guarda no Executor.
        executor.figuraAtual = Elipse(coordenadas, corBorda, corPreenchimento, dash=(4,2))
    ...
    def incluir_figura_nova(self, event, executor):
        if executor.figuraAtual is not None:
            #Finalizando a elipse(tira o tracejado - dash):
            executor.figuraAtual.finalizar()
            
            #Guardando a elipse na lista de desenhos do Executor:
